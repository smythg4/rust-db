...
- [x] Add a `Bool` type for `RowValues`, don't allow it as a Primary Key in the `Schema`.
...
- [x] Add an error return if someone tries to construct a `Row` with an invalid type in the primary key (first) slot.
